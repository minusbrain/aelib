...
EventHandlerExpression::EventHandlerExpression(
  const EventHandlerExpression&& orig)
  : _triggerEvent(orig._triggerEvent), _guards(orig._guards),
    _actions(orig._actions), _destinationState(orig._destinationState) {}
...
EventHandlerExpression::EventHandlerExpression()
  : _triggerEvent(NOTDEFINED_BM), _guards(), _actions(),
