...
class TestWorld : public ::testing::Test, public ae::IPathfindWorld<TestNode> {
  public:
    ...
    */
    ...
    virtual std::vector<std::pair<TestNode, uint32_t>>
    getDirectNeigboursAndCosts(TestNode node) {
        std::vector<std::pair<TestNode, uint32_t>> neigbourList{};

        for (int32_t row = (int32_t)node.first - 1;
             row <= (int32_t)node.first + 1; ++row) {
            for (int32_t col = (int32_t)node.second - 1;
                 col <= (int32_t)node.second + 1; ++col) {
                if (row >= 0 && row < (int32_t)WORLDSIZE && col >= 0 &&
                    col < (int32_t)WORLDSIZE) {
                    if (_world[row][col] == false)
                        continue;
                    uint32_t cost = SIMPLE_TRAVELCOST;
                    if (row != (int32_t)node.first &&
                        col != (int32_t)node.second) {
                        cost = DIAGONAL_TRAVELCOST;
                    }
                    neigbourList.push_back(
                      std::pair<TestNode, uint32_t>{{row, col}, cost});
                }
            }
        }
...
TEST_F(TestWorld, EmptyWorld_SimpleAtoBDiagonal_CorrectCost) {
    ...
    ae::Path<TestNode> path =
      ae::Pathfinder<TestNode>::findPath(this, {1, 1}, {8, 8});
...
TEST_F(TestWorld, EmptyWorld_SimpleAtoBStraight_CorrectCost) {
    ...
    ae::Path<TestNode> path =
      ae::Pathfinder<TestNode>::findPath(this, {0, 0}, {0, 9});
...
TEST_F(TestWorld, BarrierWorld_AtoB_CorrectCost) {
    ...
    ae::Path<TestNode> path =
      ae::Pathfinder<TestNode>::findPath(this, {6, 0}, {1, 9});
...
TEST_F(TestWorld, CrossWorld_AtoB_CorrectCost) {
    setWorldCross();
    ae::Path<TestNode> path =
      ae::Pathfinder<TestNode>::findPath(this, {4, 4}, {6, 6});
