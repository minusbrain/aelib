...
class TestWorld : public ::testing::Test, public base::IPathfindWorld<TestNode>
{
    ...
    */
    ...
    virtual std::vector<std::pair<TestNode, uint32_t>> getDirectNeigboursAndCosts(
        TestNode node)
    {
        ...
        {
            for (int32_t col = (int32_t)node.second - 1; col <= (int32_t)node.second + 1;
                 ++col)
            {
    ...
    {
        return std::round(
            std::sqrt(std::pow(static_cast<float>(node1.first) - node2.first, 2.0) +
...
TEST_F(TestWorld, EmptyWorld_SimpleAtoBDiagonal_CorrectCost)
{
    ...
    base::Path<TestNode> path =
        base::Pathfinder<TestNode>::findPath(this, {1, 1}, {8, 8});
...
TEST_F(TestWorld, EmptyWorld_SimpleAtoBStraight_CorrectCost)
{
    ...
    base::Path<TestNode> path =
        base::Pathfinder<TestNode>::findPath(this, {0, 0}, {0, 9});
...
TEST_F(TestWorld, BarrierWorld_AtoB_CorrectCost)
{
    ...
    base::Path<TestNode> path =
        base::Pathfinder<TestNode>::findPath(this, {6, 0}, {1, 9});
...
TEST_F(TestWorld, CrossWorld_AtoB_CorrectCost)
{
    setWorldCross();
    base::Path<TestNode> path =
        base::Pathfinder<TestNode>::findPath(this, {4, 4}, {6, 6});
