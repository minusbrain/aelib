...
{
   public:
    virtual std::vector<std::pair<T_Node, uint32_t>> getDirectNeigboursAndCosts(
        T_Node) = 0;
...
class Pathfinder
{
    ...
    static Path<T_Node> findPath(IPathfindWorld<T_Node>* world, T_Node dest, T_Node start)
    {
        ...
        while (openList.size() > 0)
        {
...
#ifdef DEBUG_FLAG
            std::cout << "Current Node: (" << current.first << " | " << current.second
                      << ")" << std::endl;
#endif
            ...
            for (auto neigbour : neigbours)
            {
                ...
                if (std::find(openList.begin(), openList.end(), neigbour.first) ==
                    openList.end())
                {
                ...
                {
                    metaLUT[neigbour.first] =
                        Metadata{std::numeric_limits<uint32_t>::max(),
   ...
   private:
    struct Metadata
    {
        ...
        Metadata(uint32_t pg, uint32_t pf, std::optional<T_Node> pcameFrom)
