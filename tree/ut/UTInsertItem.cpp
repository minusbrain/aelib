...
void genPngImage(const ae::Tree<int>& tree, std::string text, ae::Tree<int>::iterator it) {
	static int count = 0;
	...
	command << "\"C:\\Program Files (x86)\\Graphviz2.38\\bin\\dot.exe\" -Tpng " << filenameDot.c_str() << " > " << filenamePng.replace(filenamePng.find(".dot"), filenamePng.size(), ".png").c_str();
