...
namespace ae {
	...
	class Tree {
	public:
		typedef ae::Iterator<T> iterator;
...
#ifdef _AE_TREE_DEBUGMODE_
		void setDebugCallback(std::function<void(const Tree<T>&, std::string, iterator)> callback) {
			_dbgcb = callback;
		}
#endif
...
#endif
			if (lc != nullptr) { ++children; child = lc;}
			if (rc != nullptr) { ++children; child = rc;}
		...
		iterator find(const T& item) const {
			return recursiveFind(_root, item);
		}
		...
		iterator begin() const {
			return Iterator<T>(getLeftMostNode(_root));
		}
